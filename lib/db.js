    ...
    schemaPromise = (async () => {
      ...
      await sql.query(`
        ...
        BEGIN
          ...
          IF old_count > 0 THEN
            UPDATE users
            SET score=GREATEST(score - old_count, 0), updated_at=NOW()
...
async function ensureGameSchema() {
  if (!gameSchemaPromise) {
    gameSchemaPromise = (async () => {
      ...
      await sql.query(
        "CREATE TABLE IF NOT EXISTS tuz_rolls (chat_id BIGINT NOT NULL, user_id BIGINT NOT NULL, last_rolled_at TIMESTAMPTZ NOT NULL, PRIMARY KEY(chat_id,user_id))"
      );
      await sql.query(
      ...
      await sql.query(`
        CREATE OR REPLACE FUNCTION apply_tuz_roll(
          p_chat_id BIGINT,
          p_user_id BIGINT,
          p_username TEXT,
          p_first_name TEXT,
        ...
        RETURNS TABLE(applied_delta INTEGER, new_score INTEGER, seconds_remaining INTEGER)
        LANGUAGE plpgsql
        AS $
        DECLARE
        ...
        BEGIN
          PERFORM pg_advisory_xact_lock(
            hashtextextended('roll:' || p_chat_id::text || ':' || p_user_id::text, 0)
          );
          ...
          IF FOUND AND v_last_roll > NOW() - INTERVAL '24 hours' THEN
            ...
            v_remaining := GREATEST(
              CEIL(EXTRACT(EPOCH FROM (v_last_roll + INTERVAL '24 hours' - NOW())))::int,
              0
            );

            RETURN QUERY SELECT 0, COALESCE(v_current_score, 0), v_remaining;
            RETURN;
          END IF;
          ...
          IF p_requested_delta < 0 THEN
            v_actual_delta := -LEAST(ABS(p_requested_delta), GREATEST(v_current_score, 0));
          ELSE
            v_actual_delta := GREATEST(p_requested_delta, 0);
          END IF;

          UPDATE users
          SET score=GREATEST(score + v_actual_delta, 0), updated_at=NOW()
          WHERE chat_id=p_chat_id AND user_id=p_user_id
          RETURNING score INTO v_new_score;

          INSERT INTO tuz_rolls(chat_id,user_id,last_rolled_at)
          ...
          VALUES(p_chat_id,p_user_id,p_kind,p_rolled_value,v_actual_delta,NOW());

          RETURN QUERY SELECT v_actual_delta, v_new_score, 0;
        END;
        $
      `);
    })();
...
export async function applyTuzRoll(chatId, user = {}, kind, rolledValue, requestedDelta) {
  await ensureGameSchema();
  const rows = await sql.query(
    "SELECT * FROM apply_tuz_roll($1,$2,$3,$4,$5,$6,$7,$8)",
    [
      chatId,
      user.id,
      user.username || null,
  ...
  return rows[0] || { applied_delta: 0, new_score: 0, seconds_remaining: 0 };
}
