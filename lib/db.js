  ...
  const rows = await sql.query(
    `WITH locked AS MATERIALIZED (
     ...
     roll AS MATERIALIZED (
       SELECT
         available,
         (available > 0 AND random() < 0.05) AS jackpot
       FROM locked
     ),
     vault_update AS (
       UPDATE tuz_vaults v
       ...
       FROM roll
       WHERE v.chat_id=$1 AND v.user_id=$2 AND roll.available>0
       RETURNING v.user_id
     ),
     user_update AS (
       UPDATE users u
       SET score=u.score +
         roll.available * CASE WHEN roll.jackpot THEN 2 ELSE 1 END,
         updated_at=NOW()
       FROM roll
       WHERE u.chat_id=$1 AND u.user_id=$2 AND roll.available>0
       RETURNING u.score
     )
     SELECT
       roll.available::int AS collected,
       roll.jackpot,
       (
         roll.available * CASE WHEN roll.jackpot THEN 2 ELSE 1 END
       )::int AS payout,
