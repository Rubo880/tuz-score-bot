...
export const MAX_TUZ_PER_MINUTE = 7;
...
export async function ensureSchema() {
  if (!schemaPromise) {
    schemaPromise = (async () => {
      ...
      await sql("CREATE TABLE IF NOT EXISTS score_events (id BIGSERIAL PRIMARY KEY, chat_id BIGINT NOT NULL, user_id BIGINT NOT NULL, message_id BIGINT NOT NULL, delta INTEGER NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW())");
      await sql("CREATE INDEX IF NOT EXISTS users_chat_score_idx ON users(chat_id, score DESC)");
      await sql("CREATE INDEX IF NOT EXISTS score_events_user_time_idx ON score_events(chat_id, user_id, created_at DESC)");
      await sql(`
        ...
        BEGIN
          ...
          IF p_raw_count > 3 THEN
            desired_count := 0;
          ELSE
            desired_count := GREATEST(p_raw_count, 0);
          END IF;

          delta := desired_count - old_count;

          -- Anti-cheat #2: at most 7 newly-awarded points per user per rolling 60 seconds.
          IF delta > 0 THEN
            ...
            allowed_points := GREATEST(7 - recent_points, 0);
